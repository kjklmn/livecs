...
Java_com_nan_live_pusher_PushNative_fireVideo(JNIEnv *env, jobject instance, jbyteArray buffer_) {
    ...
    //视频数据NV21转为YUV420P
    /*在我们的java代码中，我们设置camera预览的ImageFormat为NV21，而NV21从android源码中可以看到是YCrCb，即YVU顺序，
     * 所以在这里要把YVU的顺序转成YUV顺序，即把NV21->YUV420P：NV21是在java端camera预览图像的像素格式
     * x264_picture_t 结构体是用来描述一视频帧的特征，而此结构体中的数据x264_image_t（也是一个接头体）用于存放一帧图像实际像素数据
     * x264_image_t中有一个二维数组uint8_t *plane[4]，plane[0]是Y平面，plane[1]和plane[2]分别代表U和V平面
     */
    jbyte *nv21_buffer = (*env)->GetByteArrayElements(env, buffer_, NULL);
    LOGI("%d", sizeof(*nv21_buffer));
    jbyte *u = pic_in.img.plane[1];//赋值给字节数组u只是为了后面操作方便
    jbyte *v = pic_in.img.plane[2];//赋值给字节数组v也只是为了后面操作方便
    ...
    //还可以结合OpenCV识别人脸等等
    /*
     * 把这帧视频数据nv21_buffer的前面y_len个字节拷贝到plane[0]Y平面中
     */
    memcpy(pic_in.img.plane[0], nv21_buffer, y_len);
    int i;
    for (i = 0; i < u_len; i++) {
        *(u + i) = *(nv21_buffer + y_len + i * 2 + 1);//把当前这帧YVU数据的U数据赋值给pic_in.img.plane[1]
        *(v + i) = *(nv21_buffer + y_len + i * 2);//把当前这帧YVU数据的v数据赋值给pic_in.img.plane[2]
    }

    //h264编码得到NALU数组
    x264_nal_t *nal = NULL; //NAL：用来存放编码得到的数据
    int n_nal = -1; //NALU的个数
    /*进行h264编码:编码完成之后得到一系列的nal单元，即nal数组，在这里是一个x264_nal_t数据
     * nal单元的讲解在01:27:43
     * */
    ...
    memset(sps, 0, 100);//将sps所指向的某一块内存的前100个字节的内容全部设置为0指定的ASCII值
    memset(pps, 0, 100);
    pic_in.i_pts += 1; //顺序累加
    //遍历NALU数组，根据NALU的类型判断
    /*
     * 这里对nal数组进行遍历，nal数组的类型有三种情况：NAL_SPS，NAL_PPS，还有就是其他情况
     */
    for (i = 0; i < n_nal; i++) {
